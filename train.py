...
def train(sess,loss_op,train_op,X,Y,train_X,train_Y,val_X,val_Y,prediction, last_state,fout_log):
    ...
    for epoch in range(1):
        for step in range(train_size):
            ...
            X_in,Y_in=copy_init_snapshot(train_X[step], train_Y[step],3,10)
            train=sess.run(train_op, feed_dict={X:X_in, Y:Y_in})
            # Calculate batch loss in validation set
            if stepp%50 == 0:
                val_sum=0
             #   for j in range(val_size):
                j=random.randint(0,val_size-1)
                X_val,Y_val=copy_init_snapshot(val_X[j], val_Y[j],3,10)
                lossv= 10000 * sess.run(loss_op,  feed_dict={X:X_val, Y:Y_val})
                val_sum=lossv
                loss = val_sum
                #Calculated running average of val_loss
                if stepp == 50:  # loss start from very large val at step=0, so start from 10th step
                    val_loss = loss
                elif stepp >50:
